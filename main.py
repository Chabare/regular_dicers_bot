...
def run_scheduler(bot: Bot):
    ...
    additional_reset_time: str = default_timezone.localize(today.replace(hour=12, minute=30)).strftime("%H:%M")
    attend_time: str = default_timezone.localize(today.replace(hour=13, minute=0)).strftime("%H:%M")
    dice_time: str = default_timezone.localize(today.replace(hour=19, minute=30)).strftime("%H:%M")
